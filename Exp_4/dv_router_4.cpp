...
dvUpdate() {
    ...
    int src, trg, rec, tmp, convergeFlag = 1;
    for (src = 0; src < rtnum; src++) {
        for (trg = 0; trg < rtnum; trg++) {
            if (rttable[src][trg].isAdj == true) {
                for (rec = 0; rec < rtnum; rec++) {
                    /* Poison Reverse */
                    if(rttable[trg][rec].next == src) {
                        tmp = rttable[trg][rec].dist;
                        rttable[trg][rec].dist = _POS_INF_;
                    }
                    if((rttable[src][rec].dist <= _MAX_NOD_DST_ && rttable[src][rec].next == trg)) {
